using System;
using SimpleGraphQL.GraphQLParser.Cache;
using SimpleGraphQL.GraphQLParser.Exceptions;
        ...
        private string AppendToValueByCode(string value, char code)
        {
            switch (code)
            {
                ...
                case 't':
                    ...
                    break;
                case 'u':
                    value = GetUnicodeChar().ToString();
                    break;
                default:
                    throw new GraphQLSyntaxErrorException($"Invalid character escape sequence: \\{code}.", _source,
                        _currentIndex);
            }
...
//                        break;
        ...
        private char GetUnicodeChar()
        {
            if (_currentIndex + 5 > _source.Body.Length)
            {
                ...
                throw new GraphQLSyntaxErrorException(
                    $"Invalid character escape sequence at EOF: \\{truncatedExpression}.", _source, _currentIndex);
            }
            ...
            {
                throw new GraphQLSyntaxErrorException($"Invalid character escape sequence: \\{expression}.", _source,
                    _currentIndex);
            }
        ...
        {
            while (++position < body.Length && (code = body[position]) != 0 && (code > 0x001F || code == 0x0009) &&
